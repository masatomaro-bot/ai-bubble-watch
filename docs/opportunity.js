...
const D=window.OpportunityDiscovery;
...
function reportFor(ticker){
 ...
 if(candidate)return {...candidate,caution:false,signals:candidate.reasons.map(text=>({text})),limitations:[candidate.researchStatus],next:'直近決算の成長・利益・現金収入を照合し、競争優位の根拠と壊れる条件を検証する。'};
 return discovery.byTicker[ticker];
}
...
function render(){const root=document.getElementById('opportunity');if(!root)return;root.querySelectorAll('form[data-dirty=true]').forEach(f=>drafts.set(draftKey(f),{values:Object.fromEntries(new FormData(f)),reportId:f.dataset.report}));const latest=window.watchLatest,prev=window.watchPrevious;discovery=R.derive(latest,prev?.data);offenseDiscovery=D?.select(latest,prev?.data);const today=new Date().toLocaleDateString('sv-SE');const diff=latest?changes(latest,prev?.data):[];const selected=data.cards.filter(c=>c.lane===lane).sort((a,b)=>Number(!!reportFor(b.ticker))-Number(!!reportFor(a.ticker)));const queue=selected.filter(c=>priority(c,today)<4).sort((a,b)=>priority(a,today)-priority(b,today)||(a.date||'9999').localeCompare(b.date||'9999')).slice(0,3);const age=latest?Math.floor((Date.now()-Date.parse(latest.date+'T00:00:00Z'))/86400000):null;
root.innerHTML=`<div class="card op-hero"><div class="op-eyebrow">OPPORTUNITY WATCH / 攻めの準備</div><h2>市場の変化から、調べる企業を選ぶ。</h2><p>地合いウォッチと同じデータから候補を発見し、業績と競争優位の裏付けを検証する。</p><p class="op-note">入口はデータ。企業の稼ぐ力と、成長を維持できる理由を調べるための画面です。</p><p class="op-note">${latest?`地合い：${e(({correction:'調整局面',uptrend_under_pressure:'圧力下の上昇トレンド',confirmed_uptrend:'確認された上昇トレンド'})[latest.overall_trend_state]||latest.overall_trend_state)} / データ日 ${e(latest.date)} / 自動取得・未検証`:'市場データを取得できていません。判断カードは利用できます。'}${age>3?' / データが古いため最新状況を確認してください。':''}</p></div>
${D?D.render(offenseDiscovery,S,data.cards):'<p class="op-alert">候補抽出を読み込めませんでした。再読み込みしてください。</p>'}
<details class="card op-story-archive"><summary>保存済みの企業資料（自動選定とは別）</summary><p class="op-note">固定の5社の資料です。今日の候補入りや、分析の再検証を意味しません。</p>${S?S.render(storyTicker,data.cards):'<p>企業資料を読み込めませんでした。</p>'}</details>
...
root.querySelectorAll('[data-story]').forEach(b=>b.onclick=()=>{storyTicker=b.dataset.story;render();const archive=root.querySelector('.op-story-archive');if(archive)archive.open=true;document.getElementById('story-detail')?.focus();});
